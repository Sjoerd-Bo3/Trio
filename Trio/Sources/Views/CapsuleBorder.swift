...
// Capsule (pill-shaped) border helpers, built on the shared `AnimatedBorder`
// engine in AnimatedBorder.swift. Used by the loop and pump reservoir pills.
extension View {
    ...
    func spinningCapsuleBorder(isActive: Bool, color: Color, lineWidth: CGFloat = 2) -> some View {
        modifier(AnimatedBorder(isActive: isActive, color: color, lineWidth: lineWidth, cornerRadius: nil))
    }
